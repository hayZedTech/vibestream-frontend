...
export default function Feed({ isProfile = false }) {
  ...
  // Some projects have API.baseURL = "http://host:port/api" and others "http://host:port".
  // To be robust, try a short list of auth endpoints until one works.
  const tryLoadCurrentUser = async () => {
    ...
    for (const p of tries) {
      try {
        ...
        if (res && res.data) {
          // expect res.data to contain at least { username, id? } or similar
  ...
  useEffect(() => {
    ...
    (async () => {
      // If we already have a username in state (from localStorage), use it
      if (loggedInUsername) {
      ...
      // no server user found — do not auto-set anything, user can set a quick name (dev mode)
    })();
  ...
  useEffect(() => {
    if (!loggedInUsername) {
      // if no username, don't attach chat listeners because joining not done
      return;
    }

    // ensure socket connected
    if (!socket.connected) socket.connect();
    // join mapping on server
    ...
    socket.on("chatMessage", (msg) => {
      // dedupe & move to front for recent messages
      ...
      // append to conversation if it belongs or if no chatTo set
  ...
  const fetchConversation = async (userA, userB) => {
    ...
    try {
      ...
      // mark read on server and update recent locally
      try {
  ...
  const sendMessage = async () => {
    ...
    if (!loggedInUsername) {
      // instead of silent alert, show inline guidance and a toast
    ...
    try {
      ...
      // fallback to socket emit (server may persist it there too)
      try {
  ...
  // if chat target changes, load conversation
  ...
  // small dev helper: set name manually (also connects socket and stores in localStorage)
  const setQuickName = () => {
    ...
    <div className="container mt-4" style={{ maxWidth: 800 }}>
      <div className="d-flex align-items-center justify-content-between mb-3">
        ...
        <div className="d-flex align-items-center">
          <div className="me-2">
            <NotificationsDropdown notifications={notifications} onClickNotif={handleNotificationClick} />
          </div>

          <div className="position-relative me-2">
            <button className="btn btn-outline-primary btn-sm" onClick={() => { setShowInboxList((s) => !s); }} title="Inbox">✉️</button>
...
// NotificationsDropdown helper (kept local to file)
function NotificationsDropdown({ notifications, onClickNotif }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="position-relative">
      <button className="btn btn-secondary btn-sm" onClick={() => setOpen(o => !o)} style={{ position: "relative" }}>
        🔔
